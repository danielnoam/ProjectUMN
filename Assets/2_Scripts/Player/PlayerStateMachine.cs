...
public class PlayerStateMachine : MonoBehaviour, Iinteractor
{
    ...
    [Header("AimRay")]
    [SerializeField] private float aimRayMaxDistance = 45f;
    ...
    private readonly Vector3 _crouchCharacterCenter = new Vector3(0, -0.34f, 0f);
    private float _rotatingToTargetTimer;
    private Vector3 _lastCameraForward;
    private bool _isMovingLaterally;
    private bool _lastGroundedState;
    private Vector3 _lastMoveDirection = Vector3.zero;
    private bool _wasMovingLastFrame;
    private bool _isRotatingFromIdle;
    ...
    private void UpdateLineRenderer()
    {
        ...
        else if (!IsAiming && _isLineVisible)
        {
            ...
            // to zero length, which happens in the leaping code below
    ...
    public void HandleRotation(bool allowRotation, bool alignWithCameraWhenIdle)
    {
        ...
        if (IsAiming)
        {
            ...
            else if (_isMovingLaterally)
            {
                ...
                // Calculate angle difference between current and target rotation
                float angleDifference = Quaternion.Angle(transform.rotation, targetRotation);
    
                // Set IsRotatingToTarget based on whether we still need significant rotation
                IsRotatingToTarget = angleDifference > 5.0f; // Use appropriate threshold value
    ...
    public void CheckForRippleEffect()
    {
    ...
    {
        jumpSfx?.PlayAtPoint(transform.position - Vector3.down);
    }
