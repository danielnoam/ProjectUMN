...
public class PositionedBillboard : MonoBehaviour
{
    ...
    public float movementSmoothTime = 0f; // Lower values = faster movement
    ...
    public float rotationSmoothTime = 0.2f; // Lower values = faster rotation
    ...
    private void Update()
    {
        if (!centerObject || !_mainCamera) return;
    ...
    private void UpdatePosition(bool instant)
    {
        ...
        else
        {
            transform.position = Vector3.SmoothDamp(transform.position, _targetPosition, 
                                                  ref _currentVelocity, movementSmoothTime);
        }
    }
